...
class Detector(object):
    ...
    def predict(self,label_class,repeats=1):
        '''
        Args:
        ...
        for key, vlaue in result.items():
            if key == 'boxes':
                # for i in range(len(vlaue)):
                #     if int(vlaue[i][0]) == 3 and vlaue[i][1] > 0.5:
                #         car_boxs.append(vlaue[i])
                for i in range(len(vlaue)):
                    for label in range(len(label_class)):
                        if int(vlaue[i][0]) == label_class[label] and vlaue[i][1] > 0.5:
                            car_boxs.append(vlaue[i])


        car_boxes_num.append(len(car_boxs))
        car_boxs = np.array(car_boxs,dtype=np.float32)
        car_boxes_num = np.array(car_boxes_num)
        result = dict(boxes=car_boxs, boxes_num=car_boxes_num)
        return result
